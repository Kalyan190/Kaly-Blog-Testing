      ...
      <main className='container mx-auto px-4 py-12 sm:px-6 lg:text-5xl'>
          ...
          <div className='mt-12 flex justify-center gap-2'>
              <Link href={`?search=${searchText}&page=${currentPage-1}`} passHref>
                 <Button className='cursor-pointer' disabled={currentPage === 1} variant={'ghost'} size={'sm'}>← Prev</Button>
              </Link>
              {
               Array.from({length:totalPages}).map((_, index)=>(
                  <Link key={index} href={`?search=${searchText}&page=${index + 1}`} passHref>
                     <Button className='cursor-pointer' variant={`${currentPage === index + 1 ? 'destructive': 'ghost'}`} size={'sm'}>{index + 1}</Button>
                  </Link>
               ))
              }
              
             <Link href={`?search=${searchText}&page=${currentPage + 1}`} passHref>
                 <Button className='cursor-pointer' disabled={currentPage === totalPages} variant={'ghost'} size={'sm'}>Next →</Button>
             </Link>
          </div>
      </main>
   </div>
